...
const handleInput = debouce(function () {
  nbVariable = Number(variableInput.value);
  addInputs(equationContainer, nbVariable);
}, 250);

variableInput.addEventListener("input", handleInput);

variableInput.addEventListener("change", handleInput);
...
resolveButton.addEventListener("click", async function () {
  ...
  equationsInfoArr = Array.prototype.map.call(
    equationContainer.querySelectorAll("input"),
    function (input) {
      //input.disabled = true;
      return {
        input: input,
      ...
      if (typeof errors == "string") {
        ...
        for (const key in errors) {
          if (Object.hasOwnProperty.call(errors, key)) {
            equationsInfoArr[Number(key)].input.nextElementSibling.innerHTML =
              errors[key];
          }
        }
      }
